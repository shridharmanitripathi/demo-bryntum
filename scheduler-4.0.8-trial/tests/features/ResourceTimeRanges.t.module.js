import { Scheduler, ArrayHelper, DateHelper, Rectangle, ResourceTimeRangeStore } from '../../build/scheduler.module.js?447702';
...
StartTest(t => {
    ...
    async function createScheduler(count = 50) {
    ...
    t.it('Should render ranges correctly initially', async t => {
        ...
        checkBounds(t, eventElements[17], { left : 900, top : 1230, width : 200, height : 40 });
    ...
    t.it('Should render ranges correctly when scrolled into view', async t => {
        ...
        t.chain(
            ...
            next => {
                const rangeElements = Array.from(document.querySelectorAll('.b-sch-resourcetimerange:not(.b-sch-released)'));
    ...
    t.it('Remove should update UI', async t => {
        ...
        t.chain(
            {
                waitFor : () => document.querySelectorAll(rangeSelector).length === 1,
                desc    : 'Waiting for range element to fade away'
            },

            next => {
                const rangeElements = Array.from(document.querySelectorAll(rangeSelector));
    ...
    t.it('Remove all should update UI', async t => {
        ...
        t.chain(
            {
                waitFor : () => document.querySelectorAll(rangeSelector).length === 0,
    ...
    t.it('Should draw resource time ranges after events are filtered', async t => {
        ...
        t.chain(
            ...
            async() => {
                ...
                t.is(
                    timeRangesCount,
                    document.querySelectorAll('.b-sch-timeaxis-cell').length,
                    'Resource time range for every resource'
                );
