import { Scheduler, ObjectHelper, VersionHelper } from '../../build/scheduler.module.js?447702';
...
StartTest(t => {
                ...
                eventContextMenu : {
                    ...
                    processItems({ eventRecord, items }) {
                        if (eventRecord === scheduler.eventStore.getAt(1)) {
                            ...
                            t.is(ObjectHelper.getTruthyKeys(items).filter(ref => !items[ref].hidden).length, 4, '4 visible items (2 default, one extra on feature config, one added dynamically)');
                        }
    ...
    t.it('LEGACY: Should be possible to trigger menu using API', t => {
        ...
        let menu = scheduler.features.eventContextMenu,
