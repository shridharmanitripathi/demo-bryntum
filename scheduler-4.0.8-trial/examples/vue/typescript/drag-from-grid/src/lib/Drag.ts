import { DragHelper, DomHelper, Rectangle, WidgetHelper, Grid, Scheduler, Store } from "bryntum-scheduler";
...
export default class Drag extends DragHelper {
    ...
    public onTaskDragStart({ context }: { context: any }) {
        const
            me = this,
            mouseX = context.clientX,
            proxy = context.element,
            task: any = me.grid.getRecordFromElement(context.grabbed),
    ...
    public onTaskDrag({ context }: { context: any }) {
        const me = this,
    ...
    public onTaskDrop({ context }: { context: any }) {
        const me = this,
            task = context.task,
        ...
        if (context.valid && target) {
            ...
            if (date) {
                ...
                (me.grid.store as Store).remove(task);
