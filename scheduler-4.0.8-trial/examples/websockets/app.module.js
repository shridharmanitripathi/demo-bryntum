import { WidgetHelper, DomHelper, EventModel, Toast, Scheduler } from '../../build/scheduler.module.js?447702';
...
class WebSocketHelper {
    ...
    setVisibility(cls, visible) {
        DomHelper.forEachSelector(cls, element => {
    ...
    onEventStoreChange(event) {
        const
            me                                   = this,
            { action, record, records, changes } = event;
        ...
        switch (action) {
            case 'update' : {
                ...
                me.wsSend({ command : 'updateEvent', id : record.id, changes : data });
                break;
            }
    ...
    onEventPartialResize({ eventRecord, startDate, endDate, context }) {
        this.wsSend({
            command : 'resizeEvent',
            id      : eventRecord.id,
            startDate,
