...
StartTest(t => {
    ...
    const
        ...
        wsHelper  = scheduler.webSocketHelper;

    // To be able to detect Toasts reliably
    ...
    wsHelper._socket.onopen();
    // wsSend({ command : 'hello', userName : 'Henry' });
    ...
    t.it('Testing event drag', t => {
        ...
        t.chain(
            { drag : '[data-event-id="1"]', by : [scheduler.timeAxisViewModel.tickSize, 0] },
            () => {
                rec = sentData[0];
    ...
    t.it('Testing event resize', t => {
        t.chain(
            { drag : '[data-event-id="1"]', offset : ['100%-5', '50%'], by : [100, 0] },
            () => {
                rec = sentData[0];
    ...
    t.it('Testing event create', t => {
        t.chain(
            { drag : '.b-sch-timeaxis-cell', offset : [200, 20], by : [100, 0] },
    ...
    t.it('Testing emulating receive create event', async t => {
        ...
        wsHelper.wsReceive({ command : 'addEvent', userName : 'Test', records : [event] });
    ...
    t.it('Testing logout', t => {
        t.chain(
            ...
            { waitForElementVisible : '.b-button:contains(Login)', desc : 'Login button visible' },
            { waitForElementVisible : '.b-mask:contains(OFFLINE)', desc : 'mask shown' },
