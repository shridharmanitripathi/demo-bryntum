    ...
    methods : {
        ...
        assertDependency(scheduler, dependency, { fromSide, toSide, fromBox, toBox } = {}, msg = 'Assert dependency') {
            ...
            if (from && to) {
                ...
                this.isApprox(depFromPoint[0], fromPoint[0], 1, `${msg}: Start point X is correct (${from.name})`);
                this.isApprox(depFromPoint[1], fromPoint[1], 1, `${msg}: Start point Y is correct (${from.name})`);

                this.isApprox(depToPoint[0], toPoint[0], 1, `${msg}: End point X is correct (${to.name})`);
                this.isApprox(depToPoint[1], toPoint[1], 1, `${msg}: End point Y is correct (${to.name})`);
            }
        },
