StartTest(t => {
    ...
    t.it('Should reschedule tasks when there is an overlap (based on button pressed state)', t => {
        const
            droppedOnTask = eventStore.find(event => event.name.match('Conference')),
            startDate     = droppedOnTask.startDate;

        // Trying to fix test, not sure why it does not reschedule
        scheduler.autoRescheduleTasks = true;
        t.firesOnce(eventStore, 'refreshPreCommit');
        t.wontFire(eventStore, 'addPreCommit', 'Should batch store changes');
        // Removed `wontFire('update')`, since ending the batch makes engine normalize changes, which triggers 'update'

        const
            eventElCount = scheduler.timeAxisSubGridElement.querySelectorAll(scheduler.unreleasedEventSelector).length;
        ...
        t.chain(
            ...
            async() => {
                const
                    draggedTask = eventStore.find(event => event.name === 'Gym');

                t.expect(draggedTask.startDate).toEqual(startDate);
                t.expect(droppedOnTask.startDate).toEqual(draggedTask.endDate);

                // https://app.assembla.com/spaces/bryntum/tickets/8663
                // No duplicate elements.
                t.selectorCountIs(scheduler.unreleasedEventSelector, scheduler.timeAxisSubGridElement, eventElCount + 1);
            },
            { click : '.reschedule-button', desc : 'Restore button state' }
        );
    });

    t.it('Should NOT reschedule tasks when there is an overlap if button is not pressed', t => {
        const
            droppedOnTask = eventStore.find((event) => event.name.match('Arrange'));
        ...
        t.chain(
            ...
            () => {
                const
                    draggedTask = eventStore.find(event => event.name.match('boring'));
    ...
    t.it('Should scroll timeline when task is dragged from external grid', t => {
        t.chain(
            ...
            { drag : '[data-ref=unplanned] .b-grid-cell', to : '.b-scheduler', toOffset : ['100%-20', '50%'], dragOnly : true },
