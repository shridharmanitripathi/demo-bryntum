import { EventModel, ResourceModel, Scheduler } from '../../build/scheduler.module.js?447702';
...
StartTest(t => {
    ...
    t.it('Should be possible to dragcreate events', t => {
        ...
        scheduler.on({
            ...
            dragcreatestart({ source, proxyElement }) {
                t.ok(source instanceof Scheduler &&
                    proxyElement.classList.contains('b-sch-dragcreator-proxy'),
                'Correct event signature of `dragcreatestart`');

                fired.dragcreatestart++;
            },
            dragcreateend({ source, newEventRecord, resourceRecord, event, proxyElement }) {
                t.ok(source instanceof Scheduler &&
                    newEventRecord instanceof EventModel &&
                    resourceRecord instanceof ResourceModel &&
    ...
    t.it('Clock in tooltip is updated on create', async t => {
        ...
        t.chain(
            ...
            { drag : '.b-sch-header-row-1 .b-sch-header-timeaxis-cell:nth-child(2)', fromOffset : [0, 150], by : [[step, 0]], dragOnly : true },
