import { ResourceModel, EventModel, DateHelper, Scheduler, Toast, MessageDialog } from '../../build/scheduler.module.js?447702';
...
const scheduler = new Scheduler({
    ...
    eventBodyTemplate : data => `
        ${data.iconCls ? `<i class="${data.iconCls}"></i>` : ''}
        <section>
    ...
    listeners : {
        beforeEventAdd({ resources }) {
            const available = resources[0].available;

            if (!available) {
                Toast.show(`Resource not available: ${resources[0].statusMessage}`);
            }
        ...
        async beforeEventDropFinalize({ source : scheduler, context }) {
            if (scheduler.confirmationsEnabled) {
                ...
                const
                    namesInQuotes = context.draggedRecords.map(eventRecord => `"${eventRecord.name}"`),
        ...
        async beforeeventresizefinalize({ source : scheduler, context }) {
            if (scheduler.confirmationsEnabled) {
                ...
                const
                    ...
                    result      = await MessageDialog.confirm({
                        title   : 'Please confirm',
                        message : `"${eventRecord.name}" duration changed. Allow this operation?`
                    });
