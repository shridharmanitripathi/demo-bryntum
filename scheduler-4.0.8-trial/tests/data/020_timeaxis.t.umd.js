StartTest(function (t) {
  ...
  t.it('Filtering', function (t) {
    ...
    ta.filterBy(function (tick) {
      return tick.startDate.getHours() === 0;
    });
    ...
    ta.filterBy(function () {
      return false;
    });
  });
  t.it('Prevent reconfigure', function (t) {
    var ta = getTimeAxis({}, t);
    ...
    t.isFiredWithSignature(ta, 'beforereconfigure', function (_ref) {
      var source = _ref.source,
          startDate = _ref.startDate,
          endDate = _ref.endDate;
      return source === ta && startDate - new Date(2012, 1, 1) === 0 && endDate - new Date(2012, 1, 5) === 0;
    });
  ...
  t.it('floorDate', function (t) {
    t.it('SECOND', function (t) {
      var ta = getTimeAxis({
        ...
        startDate: new Date(2012, 1, 1, 1),
        endDate: new Date(2012, 1, 2)
      }, t); // the actual unit/resolution unit of the time axis does not matter for `relativeToStart = false`
    ...
    t.it('MINUTE', function (t) {
      ...
      t.expect(ta.floorDate(new Date(2012, 1, 1, 2, 1), true, 'minute')).toEqual(new Date(2012, 1, 1, 2, 0)); // the actual unit/resolution unit of the time axis does not matter for `relativeToStart = false`
  ...
  t.it('Direct and reverse lookups should work correct all along not adjusted time axis', function (t) {
    var axis = getTimeAxis({
      autoAdjust: false,
      mainUnit: 'day',
      unit: 'hour',
      increment: 8,
    ...
    var date = new Date(2013, 8, 1, 7),
        tickFromDate;

    for (var i = 1; date < axis.endDate; i++) {
  ...
  t.it('forEachAuxInterval + autoAdjust : false', function (t) {
    ...
    ta.forEachAuxInterval('hour', 1, function (start, end, i) {
      switch (i) {
        case 0:
