...
- `requestId` property contains a unique request identifier included with every request (filled automatically by the Crud Manager)
...
Crud Manager supports two basic response formats:

- _short sync response_ - enabled by default (can be changed by setting (#Scheduler/data/CrudManager#config-supportShortSyncResponse)[supportShortSyncResponse] config).
- _full sync response_ - can be enabled by setting (#Scheduler/data/CrudManager#config-supportShortSyncResponse)[supportShortSyncResponse] config to `false`.

#### Short sync response

This type of response does not require all synced records to be listed in the corresponding response.
The response should include only changes made on the server-side.
...
- `success` is a flag which indicates status of the operation. If `true`, indicates a successful response, dirty records will be marked as unmodified automatically. If `false`, operation is considered to have failed and the records continue to stay dirty and will be a part of next sync request.
...
- `rows` holds all data changes made _by the server_ that should be applied to the client.
As the bare minimum, for added records sent from the client, the server should return a combination of record _phantom identifier_ and its "real" identifier assigned by the database.
If the server decides to update _any_ other fields of _any_ record it should return an object holding a combination of the record identifier and new field values (this is shown in above snippet where server sets `added_dt` field value `2019-04-26T18:45:00` for an added `books` store record).
...
- `removed` holds ids of records removed _by the server_. Short type of response does not require records initially removed on the client to be included in response `removed` section. In this mode `removed` should hold only removals initiated on the server (due to some server logic or coming from other concurrent session etc). In the above snippet response includes removal of `authors` store record with `88888888` id initiated by the server.

#### Full response

This type of response requires all synced records to be listed in the corresponding response.
As reaction to the request described in the previous chapter we could have the following response:
...
A server can potentially implement complex logic and decide which type of response works better for a client request.
So for example it might reply with *load* response on a *sync* request when the client data is very outdated or under some other circumstances.
Or it might reply with *sync* on a *load* attempt responding with delta changes instead of all the records.
To enable this mode Crud Manager [trackResponseType](#Scheduler/crud/AbstractCrudManagerMixin#config-trackResponseType) should be enabled:
```javascript
