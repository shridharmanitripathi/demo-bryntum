import { Store, AbstractCrudManager, CrudManager, JsonEncoder, ResourceModel } from '../../build/scheduler.module.js?447702';
...
StartTest(t => {
    ...
    t.it('Should cancel load request', async t => {
        ...
        cm.load().catch(e => cancelled = true);

        // Call load once again, first request should get cancelled
        await cm.load();

        // There is no way at this point to wait for wrapped fetch promise, it is not accessible.
        // Waiting for a timeout is the ONLY way to catch possible promise rejection
    ...
    t.it('Should handle server responding garbage / 500 error', t => {
    ...
    t.it('Should handle server responding 200 but malformed JSON', t => {
        class AutoLoadCrudManager extends CrudManager {
            construct() {
